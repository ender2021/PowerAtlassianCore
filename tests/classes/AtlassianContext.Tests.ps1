...
Describe "AtlassianContext (Class)" {
    Context "5 param constructor" {
        ...
        It "constructor sets the host name correctly" {
            $jc = New-Object AtlassianContext @($user,$key,$url,$retries,$delay)
        ...
        It "constructor does not allow setting zero or lower for Retries" {
            { $jc = New-Object AtlassianContext @($user,$key,$url,0,$delay) } | Should -Throw
            { $jc = New-Object AtlassianContext @($user,$key,$url,-50,$delay) } | Should -Throw
        }
        It "constructor does not allow setting zero or lower for RetryDelay" {
            { $jc = New-Object AtlassianContext @($user,$key,$url,$retries,0) } | Should -Throw
            { $jc = New-Object AtlassianContext @($user,$key,$url,$retries,-50) } | Should -Throw
        }
    }
    Context "3 param constructor" {
        ...
        It "constructor sets the host name correctly" {
            $jc = New-Object AtlassianContext @($user,$key,$url)
